#define MotorL_I1 5     // 定義 A1 接腳（左）
#define MotorL_I2 6     // 定義 A2 接腳（左）
#define MotorL_PWML 11  // 定義 ENA (PWM調速) 接腳
#define MotorR_I3 2     // 定義 B1 接腳（右）
#define MotorR_I4 3     // 定義 B2 接腳（右）
#define MotorR_PWMR 12  // 定義 ENB (PWM調速) 接腳

// 循線模組, 請按照自己車上的接線寫入腳位
#define IRpin_LL 40
#define IRpin_L 38
#define IRpin_M 36
#define IRpin_R 34
#define IRpin_RR 32

#include"node.h"
#include"track.h"
#include"ir.hpp"
/*===========================initialize variables===========================*/
int l2 = 0, l1 = 0, m0 = 0, r1 = 0, r2 = 0;  // 紅外線模組的讀值(0->white,1->black)
int _Tp = 60;                                // set your own value for motor power
bool state = true;     // set state to false to halt the car, set state to true to activate the car
// char cmd[] = "grbrbrbrblblblbls";
char cmd[] = "grbrbrbrblblblbls";
int step = 0;
/*===========================initialize variables===========================*/

void setup() {
  // put your setup code here, to run once:
  Serial.begin(9600);

  // TB6612 pin
  pinMode(MotorL_I1, OUTPUT);
  pinMode(MotorL_I2, OUTPUT);
  pinMode(MotorR_I3, OUTPUT);
  pinMode(MotorR_I4, OUTPUT);
  pinMode(MotorR_PWMR, OUTPUT);
  pinMode(MotorL_PWML, OUTPUT);

  // tracking pin
  pinMode(IRpin_LL, INPUT);
  pinMode(IRpin_L, INPUT);
  pinMode(IRpin_M, INPUT);
  pinMode(IRpin_R, INPUT);
  pinMode(IRpin_RR, INPUT);
}

void loop() {
    if(state){  // put your main code here, to run repeatedly:
        uint8_t ir_values[N_IR];
        readIR(ir_values);
        l2 = ir_values[0];
        l1 = ir_values[1];
        m0 = ir_values[2];
        r1 = ir_values[3];
        r2 = ir_values[4];

        Serial.print(l2);
        Serial.print(l1);
        Serial.print(m0);
        Serial.print(r1);
        Serial.println(r2);


            if (l2+l1+m0+r1+r2 == 5){
            switch (cmd[step])
            {
            case 'f':
                car_front();
                // send_msg('n');
                Serial.println("Front!");
                break;
            case 'b':
                car_back();
                //   send_msg('n');
                Serial.println("Back!");
                break;
            case 'r':
                car_right();
                // send_msg('n');
                Serial.println("Right!");
                break;
            case 'l':
                car_left();
                // send_msg('n');
                Serial.println("Left!");
                break;
            case 'g':
                car_start();  
                // send_msg('n');
                Serial.println("GoGO!");
                break;
            case 's': //老實說這一個case永遠不會被呼叫到
                car_end();
                // send_msg('n');
                Serial.println("Stop!");
                state = false;
                break;
            default:
                tracking(l2, l1, m0, r1, r2);
                break;
            }
            step++;
        }
        else{
            tracking(l2, l1, m0, r1, r2);
        }
        tracking(l2, l1, m0, r1, r2);
    }
}
